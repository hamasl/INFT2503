...
public:
    ...
    btVector3 get_falling_sphere_center_of_mass(){
        return falling_sphere.body.getCenterOfMassPosition();
    }

    void set_start_pos(const float *horizontal_position, const float *vertical_position){
        transform.setOrigin(btVector3(*horizontal_position/5, *vertical_position/5, 0.0));
...
public:
    ...
    void start() {
        ...
        while (running) {
            ...
            btVector3 position = world.get_falling_sphere_center_of_mass();
            if (ImGui::SliderFloat("Horizontal ball position", &position.getX(), 0.0, 10.0)) {
                world.set_start_pos(&position.getX(), &position.getY());
                position = world.get_falling_sphere_center_of_mass();
                // Implementation needed
            }
            if (ImGui::VSliderFloat("Vertical ball position", {20, 100}, &position.getY(), 0.0, 10.0)) {
                world.set_start_pos(&position.getX(), &position.getY());
                position = world.get_falling_sphere_center_of_mass();
                // Implementation needed
            }
