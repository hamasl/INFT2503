...
//TODO window size back to 800x600
  ...
  void start() {
    ...
    //Needs to stay outside of while loop to hinder sliders resetting whenever a position ha sbeen adjusted
    float horizontal_position = 1.5;
    ...
    while (running) {
      ...
      if (ImGui::VSliderFloat("Vertical ball position", {20, 100}, &vertical_position, 0.5, 3.0)) {
        world.vertical_slide(vertical_position);
      }
      ImGui::End();
